...
\begin{frame}{アルゴリズムの設計の前に}
	...
	解決策：\alert{\sout{多項式時間で計算可能な}下界を最適解の値の代わりに用いる（下界スキーム）}
	\begin{figure}
		\centering
...
\begin{frame}{[補足] アルゴリズムの時間計算量}
	...
	\end{remark}
	\begin{itemize}
		\item マッチング辺の管理のためのデータ構造の初期化 \(\mathcal{O}(|V|)\)
		      \begin{itemize}
			      \item 各頂点に対して、マッチングに含まれているかどうかを記録する配列を用意
		      \end{itemize}
