...
\begin{frame}{前準備：再割当てにおけるコストの増加分の上界}
  \begin{lemma}
...
% \begin{frame}{安全な施設に対する削除操作}

% \end{frame}

\begin{frame}{安全でない施設 \(i\) に対する集合 \(R_i\) と施設 \(i'\)}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{figure}
        \centering
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        \item 安全でない施設 \(\alert{i} \in S\) を固定する
        \item 最適解の施設 \(i^* \in S^*\) であって、\(\phi(i^*) = \alert{i}\) を満たすものを \(R_i (\subseteq S^*)\) とする
              \begin{itemize}
                \item \(\alert{i}\) は安全でない \(\Leftrightarrow R_i \neq \emptyset\)
              \end{itemize}
        \item さらに、\(\alert{i}\) に最も近い \(R_i\) の施設を \(i^{'}\) とする
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{補足：安全でない施設 \(i \in S \bigcap S^*\) に対する集合 \(R_i\) と施設 \(i'\)}
  \begin{columns}
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        \item 安全でない施設 \(\alert{i} \in S \bigcap S^*\) を固定する
        \item 最適解の施設 \(i^* \in S^*\) であって、\(\phi(i^*) = \alert{i}\) を満たすものを \(R_i (\subseteq S^*)\) とする
              \begin{itemize}
                \item この場合、\(i \in R_i\)？
              \end{itemize}
...
\begin{frame}{追加移動で開設される施設と割当ての変化}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{figure}
        \centering
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        ...
        \item \(\localopt\) において \(i\) に割り当てられていた顧客のうち、\(\opt\) において \(i^{*}\) に割り当てられていた顧客を \(i^{*}\) に再割当てする
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{追加移動に対するコストの増加分の不等式}
  局所最適解 \(\localopt\) に対して追加操作を行なってもコストは改善しないので、

  \[
    \sum_{i \in S} f_i + \sum_{j \in D} c_{\sigma(j)j} \le \sum_{i \in S \cup \{i^*\}} f_i + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) = i^*} c_{\sigma^*(j)j} + \sum_{j \in D \colon \sigma(j) \neq i \text{ or } \sigma^*(j) \neq i^*} c_{\sigma(j)j}
  \]

  よって、
  \[
    0 \le f_{i^*} + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) = i^*} (c_{\sigma^*(j)j} - c_{\sigma(j)j})
  \]

\end{frame}

\begin{frame}{交換移動で開設される施設と割当ての変化}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{figure}
        \centering
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        \item \(i\) を閉鎖し、施設 \(i^{'}\) を開設する
        \item \(\sigma(j) = i\) である利用者 \(j\) について割当てを変更する：
              \begin{itemize}
                \item \(\sigma^*(j) \in R_i\) であれば、\(j\) を \(i^{'}\) に再割当てする
                \item \(\sigma^*(j) \notin R_i\) であれば、\(j\) を \(\phi(\sigma^*(j))\) に再割当てする
              \end{itemize}
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \note{
    \begin{itemize}
      \item 安全でない施設 \(i\) を \(S \bigcap S^*\) から選んだ場合、\(i' = i\) になる
    \end{itemize}
  }
\end{frame}

\begin{frame}{交換移動に対するコストの増加分の不等式 (1/2)}
  局所最適解 \(\localopt\) に対して交換操作を行なってもコストは改善しないので、

  \begin{align*}
    \sum_{i \in S} f_i + \sum_{j \in D} c_{\sigma(j)j} & \le \sum_{i \in S \cup \{i^{'}\} \setminus \{i\}} f_i + \sum_{j \in D \colon \sigma(j) \neq i} c_{\sigma(j)j} \\
                                                       & + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \in R_i} c_{i'j}                                \\
                                                       & + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \notin R_i} c_{\phi(\sigma^*(j))j}
  \end{align*}

  よって、
  \begin{align*}
    0 & \le  f_{i^{'}} - f_i + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \in R_i} (c_{i'j} - c_{\sigma(j)j}) \\
      & + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \notin R_i} (c_{\phi(\sigma^*(j))j} - c_{\sigma(j)j})
  \end{align*}

\end{frame}

\begin{frame}{交換移動に対するコストの増加分の不等式 (2/2)}
  さらに三角不等式より、
  \begin{align*}
    0 & \le  f_{i^{'}} - f_i + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \in R_i} (c_{i'j} - c_{\sigma(j)j})   \\
      & + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \notin R_i} (c_{\phi(\sigma^*(j))j} - c_{\sigma(j)j})      \\
      & \le    f_{i^{'}} - f_i + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \in R_i} (c_{i'j} - c_{\sigma(j)j}) \\
      & + \sum_{j \in D \colon \sigma(j) = i \text{ and } \sigma^*(j) \notin R_i} \alert{2c_{\sigma^*(j)j}}                      \\
  \end{align*}
...
\begin{frame}{施設配置問題に対する近似アルゴリズムまとめ}
  \begin{table}[]
    \centering
    \begin{tabular}{lcc}
      \toprule
