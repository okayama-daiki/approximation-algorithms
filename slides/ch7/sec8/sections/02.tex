  ...
  \only<2->{
    \begin{columns}
      \begin{column}{0.5\textwidth}
        \begin{figure}
          \centering
          \only<2>{\includegraphics[width=0.9\textwidth]{figures/lemma7-9/proof-01.drawio.png}}
          \only<3>{\includegraphics[width=0.9\textwidth]{figures/lemma7-9/proof-02.drawio.png}}
          \only<4>{\includegraphics[width=0.9\textwidth]{figures/lemma7-9/proof-03.drawio.png}}
        \end{figure}
      \end{column}
      \begin{column}{0.5\textwidth}
        \vskip 1.5em
...
\begin{frame}{開設コストの上界}
  ...
  \only<2>{
    ...
    \begin{enumerate}
      \item \(S\) の各施設を（再割り当ての補題の利用可能性の意味で）\info{安全}、\alert{安全でない}施設に分類
...
\begin{frame}{\info{安全}な施設の開設コストの上界：削除移動による不等式の導出}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{figure}
        \centering
        \only<1>{\includegraphics[width=\textwidth]{figures/lemma7-10/drop-01.drawio.png}}
        \only<2->{\includegraphics[width=\textwidth]{figures/lemma7-10/drop-02.drawio.png}}
      \end{figure}
    \end{column}
...
\begin{frame}{\alert{安全でない}施設の開設コストの上界：追加移動による不等式の導出}
  \begin{columns}
    \begin{column}{0.40\textwidth}
      \begin{figure}
        \centering
        \only<1>{\includegraphics[width=\textwidth]{figures/lemma7-10/add-01.drawio.png}}
        \only<2->{\includegraphics[width=\textwidth]{figures/lemma7-10/add-02.drawio.png}}
      \end{figure}
    \end{column}
    \begin{column}{0.50\textwidth}
...
\begin{frame}{\alert{安全でない}施設の開設コストの上界：交換移動による不等式の導出}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{figure}
        \centering
        \only<1>{\includegraphics[width=\textwidth]{figures/lemma7-10/swap-01.drawio.png}}
        \only<2->{\includegraphics[width=\textwidth]{figures/lemma7-10/swap-02.drawio.png}}
      \end{figure}
    \end{column}
    \begin{column}{0.50\textwidth}
      \only<1-2>{
...
\begin{frame}{\alert{安全でない}施設の開設コストの上界：不等式の足し合わせ (2/3)}
  ...
  \note{
    \begin{itemize}
      \item まず開設コストの項をまとめる（必ず \(i'\) は \(\phi^{-1}(i)\) に含まれるため、等号変形が可能）
...
\begin{frame}{\alert{安全でない}施設の開設コストの上界：不等式の足し合わせ (3/3)}
  \small{
    ...
    \begin{align*}
      ...
      \le & \sum_{j \colon \sigma(j) = i \text{ and } \sigma^*(j) \in \phi^{-1}(i) \setminus\{i'\}} (c_{\sigma^*(j)j} + \alert{(c_{\sigma^*(j)j} + 2c_{\sigma(j)j})} - 2c_{\sigma(j)j}) + \sum_{j \colon \sigma(j) = i \text{ and } \sigma^*(j) = i'} \alert{2c_{\sigma^*(j)j}} \\
      =   & \sum_{j \colon \sigma(j) = i \text{ and } \sigma^*(j) \in \phi^{-1}(i) \setminus\{i'\}} 2c_{\sigma^*(j)j} + \sum_{j \colon \sigma(j) = i \text{ and } \sigma^*(j) = i'} 2c_{\sigma^*(j)j}                                                                           \\
      =   & \sum_{j \colon \sigma(j) = i \text{ and } \sigma^*(j) \in \phi^{-1}(i)} 2c_{\sigma^*(j)j}
    \end{align*}
  ...
  \note{
    \begin{itemize}
      \item 最初の等式変形は、\(\sum_{j \colon \sigma(j) = i \text{ and } \sigma^*(j) \in \phi^{-1}(i)} \) の和を、\(\sigma^*(j) = i'\) とそれ以外に分けたもの
      \item 次の不等式について、
            \begin{itemize}
